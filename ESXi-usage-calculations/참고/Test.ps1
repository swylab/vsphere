...
#$cpuUsageData = $vmhost | Get-Stat -Stat cpu.usage.average | Select-Object -Property Timestamp, Value | Where-Object { ($_.Timestamp -ge $year-$month) }
$cpuUsageData | Format-Table -AutoSize

Get-VMHost | Sort Name | Select Name,
@{N="Cluster";E={($_ | Get-Cluster).Name}},
@{N="VMs";E={(Get-View $_).Vm.Count}},
@{N="Cpu.Usage.Max(%)";E={[Math]::Round((($_ | Get-Stat -Stat cpu.usage.Average | Select-Object -Property Timestamp,value | Where-Object { ($_.Timestamp -ge $startDateTime) -and ($_.Timestamp -le $endDateTime)})))}},
